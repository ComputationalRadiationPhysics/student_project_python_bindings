...
if __name__ == "__main__":
  ...
  # gpu_image = gpuMemManagement.copy_to_device(parted_images[0], parted_images[0].size, 0)
  # print(gpu_image)

  #3rd try, sampe approach as run_cupy.py, separating variables in c++ -------------------------------------------
  ...
  for i in range(0, number_of_gpus):
    partial_update[i] = gpuMemManagement.update_images_v3(update[i], parted_images[i].size, i)

  print(partial_update)

  #4th try, do evrything in C++, while learning CUDA stream ---------------------------------------------------------------------------------
