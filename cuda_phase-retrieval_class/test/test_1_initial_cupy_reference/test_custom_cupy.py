...
def test_if_real_cupy_reinterpret_ptr_is_a_gpu_array():
    cp.cuda.Device(0).use()
    a = cp.array([3.14, 4.25, 5.36], dtype=cp.float64)
    res = Test_Custom_Cupy.test_if_real_cupy_reinterpret_ptr_is_a_gpu_array(a.data.ptr)
...
def test_if_custom_cupy_reinterpret_ptr_is_a_gpu_array():
    ...
    a = cp.array([3.14, 4.25, 5.36], dtype=cp.float64)
    b = cupy_ref.Cupy_Ref(ptr = a.data.ptr, shape = a.shape, dtype = a.dtype, typestr = a.dtype.str)
    res = Test_Custom_Cupy.test_if_custom_cupy_reinterpret_ptr_is_a_gpu_array(b)
...
def test_copy_real_cupy_to_cpu():
    cp.cuda.Device(0).use()
    a = cp.array([3.14, 4.25, 5.36], dtype=cp.float64)
    b = Test_Custom_Cupy.test_copy_real_cupy_to_cpu(a.data.ptr, a.size)
...
def test_real_cupy_pointer_with_cuda_kernel():
    a = cp.array([3.14, 4.25, 5.36], dtype=cp.float64)
    b = Test_Custom_Cupy.real_cupy_increment_all_data_by_1(a.data.ptr, a.size)
...
def test_wrong_dtype_float():
    with pytest.raises(TypeError):
        ...
        b = cupy_ref.Cupy_Ref(ptr = a.data.ptr, shape = a.shape, dtype = a.dtype, typestr = a.dtype.str)
        Test_Custom_Cupy.test_wrong_dtype_float(b) #this should raise an exception
...
def test_wrong_dtype_int():
    with pytest.raises(TypeError):
        ...
        b = cupy_ref.Cupy_Ref(ptr = a.data.ptr, shape = a.shape, dtype = a.dtype, typestr = a.dtype.str)
        Test_Custom_Cupy.test_wrong_dtype_int(b) #this should raise an exception
...
def test_wrong_dtype_complex():
    with pytest.raises(TypeError):
        ...
        b = cupy_ref.Cupy_Ref(ptr = a.data.ptr, shape = a.shape, dtype = a.dtype, typestr = a.dtype.str)
        Test_Custom_Cupy.test_wrong_dtype_complex(b) #this should raise an exception
