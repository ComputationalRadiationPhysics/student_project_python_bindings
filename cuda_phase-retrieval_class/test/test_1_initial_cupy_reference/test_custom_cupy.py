...
def test_if_reinterpret_ptr_is_the_same():
    ...
    b = cupy_ref.Custom_Cupy_Ref(ptr = a.data.ptr, size = a.size, shape = a.shape, dtype = str(a.dtype)) # the type of a cupy type is a numpy type class, 
...
def test_if_custom_cupy_reinterpret_ptr_is_a_gpu_array():
    cp.cuda.Device(0).use()
    a = cp.array([3.14, 4.25, 5.36], dtype=cp.float64)
    b = cupy_ref.Custom_Cupy_Ref(ptr = a.data.ptr, size = a.size, shape = a.shape, dtype = str(a.dtype))
...
def test_copy_custom_cupy_to_custom_cupy():
    ...
    assert(b.size == c.size and b.ptr == c.ptr and b.dtype == c.dtype)
