...
__global__ void random_phase_cudastate(curandState_t *states, cufftDoubleComplex *random_phase, double *magnitude, int dimension);
...
class Phase_Algo
{
  private:
    ...
    cufftDoubleComplex *random_phase_cufft_cp,         //Sample random phase in GPU
                       *random_phase_init_cufft_cp;    //Store initial GPU random phase before iterations 
  ...
  public:
    ...
    */
      ...
      */                 
      ...
      Custom_Cupy_Ref<std::complex<T>> random_phase_cp = Custom_Cupy_Ref<std::complex<T>>::getCustomCupyRef(random_phase);
      random_phase_cufft_cp = convertToCUFFT<std::complex<T>, cufftDoubleComplex>(random_phase_cp.ptr);

      pybind11::object random_phase_init = pybind11::module::import("cupy").attr("zeros")(dimension, "dtype"_a="complex128");
      Custom_Cupy_Ref<std::complex<T>> random_phase_init_cp = Custom_Cupy_Ref<std::complex<T>>::getCustomCupyRef(random_phase_init);
      random_phase_init_cufft_cp = convertToCUFFT<std::complex<T>, cufftDoubleComplex>(random_phase_init_cp.ptr);
      ...
      */   
      ...
      do_cufft_forward(source_image_gpu_cufft_cp);
      ...
      */
      ...
      /**
      * 7. Get initial random phase, and create a copy for reset
      */
    ...
    void iterate_random_phase(int steps)
    {
      ...
      */
      ...
      {   
        do_cufft_inverse(random_phase_cufft_cp);
        do_process_arrays(random_phase_cufft_cp, mask_gpu_cp.ptr, image_output_gpu_cp.ptr, beta, mode, iter, dimension);
        do_cufft_forward(random_phase_cufft_cp);
        do_satisfy_fourier(random_phase_cufft_cp, magnitude_gpu_cp.ptr, dimension);
      }
    }
    ...
    void do_cufft_inverse(cufftDoubleComplex *data)
    {
      CUFFT_CHECK(cufftExecZ2Z(plan, data, data, CUFFT_INVERSE));
    }

    void do_cufft_forward(cufftDoubleComplex *data)
    {
      CUFFT_CHECK(cufftExecZ2Z(plan, data, data, CUFFT_FORWARD));
    }

    void do_process_arrays(cufftDoubleComplex *random_phase, T* mask, T* image_output, T beta, int mode, int iter, int dimension)
    {
      process_arrays_gpu<<<8*numSMs, 256>>>(random_phase, mask, image_output, beta, mode, iter, dimension);
    }

    void do_satisfy_fourier(cufftDoubleComplex *random_phase, T* magnitude, int dimension)
    {
      satisfy_fourier_gpu<<<8*numSMs, 256>>>(random_phase, magnitude, dimension);
    }
...
PYBIND11_MODULE(cuPhaseRet, m) 
{
  ...
  // m.def("fienup_phase_retrieval", py::overload_cast<pybind11::array_t<double, pybind11::array::c_style>, pybind11::array_t<double, pybind11::array::c_style>, int, string, double, pybind11::array_t<double, pybind11::array::c_style>>(&fienup_phase_retrieval));
  // m.def("fienup_phase_retrieval", py::overload_cast<pybind11::array_t<double, pybind11::array::c_style>, pybind11::array_t<double, pybind11::array::c_style>, int, string, double>(&fienup_phase_retrieval));
  ...
  pybind11::class_<Phase_Algo<double>>(m, "Phase_Algo", pybind11::module_local())
      .def(pybind11::init<pybind11::array_t<double, pybind11::array::c_style>, pybind11::array_t<double, pybind11::array::c_style>, std::string, double, pybind11::array_t<double, pybind11::array::c_style>>())
	    .def("get_mode", &Phase_Algo<double>::get_mode)
      .def("get_beta", &Phase_Algo<double>::get_beta)
